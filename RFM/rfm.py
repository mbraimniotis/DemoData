...
def calculate_rfm():
    ...
    """
    ride = pd.read_csv('rides.csv') # open rides.csv file
    fare = pd.read_csv('fares.csv') # open fares.csv file

    ## join rides.csv with fares.csv and filter based on rides actual revenue
    ...
    rfm_segmentation.to_csv('rfm_output.csv', sep=',')
...
def plot_distribution_over_r_f(rfm_segmentation):
    ...
    """
    _, axes = plt.subplots(nrows=1, ncols=2, figsize=(16, 12), dpi=80)
    ...
    plt.savefig('D_1_RF_Distribution.png')
    plt.show()
...
def count_number_of_customers_for_each_segment(rfm_segmentation):
    ...
    plt.savefig('D_1_Passengers_Distribution.png')
    plt.show()
